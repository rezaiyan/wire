...
import io.github.rezaiyan.wire.common.main.state.WireResult.WireError
 ...
 */
fun <T> UiState<T>.copyWithResult(value: WireResult<T>): UiState<T> {
    return when (value) {
        is WireSuccess -> copy(loading = false, exception = null, data = value.data)
        is WireError -> copy(loading = false, exception = value.exception)
    }
}
